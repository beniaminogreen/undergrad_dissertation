...
This analysis uses Google trends data as a proxy for the level of racial animus in an area.
The identifying assumption of this strategy is that Google searches for the word \wone reflect underlying racial animus in an area.
...
First, Google searches do not suffer from the same social censoring
...
If readers do not find the Google trends data a convincing proxy for racial animus, they have little reason to accept the results of this analysis.
