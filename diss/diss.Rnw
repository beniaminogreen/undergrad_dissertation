...
model_1 <- lm(word1 ~ as.factor(year) + code + sinclair_present, data = full_data)

model_2 <- lm(word1 ~ as.factor(year) + code + code:year+ sinclair_present, data = full_data)

model_3 <- lm(word1 ~ as.factor(year) + as.factor(code) + as.factor(years_before), data = full_data)
